...
bool block_is_free(struct block_id bid) {
    // >>>
    return (block_id_is_valid(bid) && ! bid.heap->is_occupied[bid.value]);
    // <<<
    // return false;
}
...
struct block_id block_allocate(struct heap* heap) {
    ...
    for (size_t i = 0; i < heap_blocks; i++) {
        ...
        if (block_is_free(bid)) {
            heap->is_occupied[bid.value] = true;
            return bid;
        }
    }
    return (struct block_id) {.valid = false, .value=0, .heap = heap};
    // <<<
}
...
void block_free(struct block_id b) {
    // >>>
    if (block_id_is_valid(b)) b.heap->is_occupied[b.value] = false;
    // <<<
}
